...
@MainActor
final class CreditwatchStoreTests: XCTestCase {

    func makeFreshStore() -> CreditwatchStore {
        // Use a fresh store; it will load persisted state or seed data.
        let store = CreditwatchStore()
        return store
    }

    func testSeedDataLoadsBelowFreeLimit() {
        let store = makeFreshStore()
        XCTAssertLessThan(store.items.count, CreditwatchStore.freeLimit)
    }

    func testAddIncreasesCount() {
        let store = makeFreshStore()
        let before = store.items.count
        let added = store.add(title: "Test Entry", category: "Core", value: 1.0)
        XCTAssertTrue(added)
    ...
    func testFreeLimitBlocksAdditionalAdds() {
        let store = makeFreshStore()
        while store.items.count < CreditwatchStore.freeLimit {
            _ = store.add(title: "Filler", category: "Core", value: 1.0)
        }
        let added = store.add(title: "Overflow", category: "Core", value: 1.0)
        XCTAssertFalse(added)
        XCTAssertEqual(store.items.count, CreditwatchStore.freeLimit)
    }
    ...
    func testProBypassesFreeLimit() {
        ...
        let added = store.add(title: "Extra", category: "Core", value: 1.0)
        XCTAssertTrue(added)
    }

    func testDeleteRemovesItem() {
        let store = makeFreshStore()
        _ = store.add(title: "ToDelete", category: "Core", value: 1.0)
    ...
    func testTotalValueSumsItems() {
        ...
        _ = store.add(title: "SumTest", category: "Core", value: 5.0)
        XCTAssertEqual(store.totalValue, before + 5.0, accuracy: 0.001)
    }

    func testToggleResolvedFlipsFlag() {
        let store = makeFreshStore()
        _ = store.add(title: "ResolveMe", category: "Core", value: 1.0)
